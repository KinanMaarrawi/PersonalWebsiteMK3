...
export default function FaultyTerminal({
    ...
    useEffect(()=>{
        const ctn=containerRef.current; if(!ctn) return;
        const renderer=new Renderer({dpr}); rendererRef.current=renderer;
        ...
        const update=t=>{
            ...
            if(pageLoadAnimation && loadAnimationStartRef.current>0){
                const dur=2000;
                const prog=Math.min((t-loadAnimationStartRef.current)/dur,1);
            ...
            if(mouseReact){
                ...
                sm.x+=(m.x-sm.x)*damping; sm.y+=(m.y-sm.y)*damping;
                const mu=program.uniforms.uMouse.value; mu[0]=sm.x; mu[1]=sm.y;
            }
    ...
    },[dpr,pause,timeScale,scale,gridMul,digitSize,scanlineIntensity,glitchAmount,flickerAmount,noiseAmp,chromaticAberration,ditherValue,curvature,tintVec,mouseReact,mouseStrength,pageLoadAnimation,brightness,bottomFade,handleMouseMove]);
